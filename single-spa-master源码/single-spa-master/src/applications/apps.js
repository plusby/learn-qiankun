...
export function getAppChanges() {
  const appsToUnload = [], // 存储未加载的子应用
    appsToUnmount = [], // 存储未挂载的子应用
    appsToLoad = [], // 存储已加载的子应用
    appsToMount = []; // 存储已挂载的子应用
  ...
  apps.forEach((app) => {
    ...
    switch (app.status) {
      case LOAD_ERROR: // 加载失败的状态并且当前子应用需要激活那么就添加到要加载的数组中
        if (appShouldBeActive && currentTime - app.loadErrorTime >= 200) {
          appsToLoad.push(app);
        }
        break;
      case NOT_LOADED: // 还未加载 或 加载资源的状态并且处于激活状态就添加到要加载的数组中
      case LOADING_SOURCE_CODE:
        if (appShouldBeActive) {
          appsToLoad.push(app);
        }
        break;
      case NOT_BOOTSTRAPPED: // 还未启动或还未挂载的状态
      case NOT_MOUNTED:
        // 没有激活并且没有加载的状态就添加到未加载的数组中
        if (!appShouldBeActive && getAppUnloadInfo(toName(app))) {
          appsToUnload.push(app);
        } else if (appShouldBeActive) { // 已经激活了就添加到要挂载的数组中
          appsToMount.push(app);
        }
        break;
      case MOUNTED: // 已经挂载了
        if (!appShouldBeActive) { // 没有激活就添加到卸载的数组中
